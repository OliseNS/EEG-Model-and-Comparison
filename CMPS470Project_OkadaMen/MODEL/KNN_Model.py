...
for channel in channels:
    ...
    print(f"Test{channel} accuracy:", test_accuracy)

    # Evaluate the model on the second test set
    print('Evaluate on Test Data from TestData.csv')
    test2_accuracy = knn.score(test2_X, test2_Y)
    print(f"Test{channel} accuracy:", test2_accuracy)

    #Generating Predictions
    predictions = knn.predict(test2_X)
    ...
    plt.savefig(f'OUTPUT/KNN/confusion_matrix_{channel}.png')  # Save the figure
    plt.close()
    ...
    plt.savefig(f'OUTPUT/KNN/Metrics_{channel}.png')  # Save the figure
    plt.close()
