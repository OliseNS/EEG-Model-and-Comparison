...
for channel in channels:
    ...
    results = ann.evaluate(test_X,test_Y)
    ...
    results2 = ann.evaluate(test2_X,test2_Y)
    print(f"test{channel} loss, test acc:", results2)

    #Generating Predictions
    probabilities = ann.predict(test2_X)
    ...
    fig.savefig(f'OUTPUT/ANN/metrics_{channel}.png')  # Save the figure
    plt.close(fig)
    plt.show()   
